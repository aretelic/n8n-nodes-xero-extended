						...
						if (options.customWhere) {
							...
							if (filters && filters.length > 0) {
								...
								for (const filter of filters) {
									...
									switch (field) {
										case 'Type':
											value = filter.typeValue as string;
											break;
										case 'Class':
											value = filter.classValue as string;
											break;
										case 'Status':
											value = filter.statusValue as string;
											break;
										case 'BankAccountType':
											value = filter.bankAccountTypeValue as string;
											break;
										case 'TaxType':
											value = filter.taxTypeValue as string;
											break;
										case 'EnablePaymentsToAccount':
											value = filter.enablePaymentsValue as boolean;
											break;
										case 'SystemAccount':
											value = filter.systemAccountValue as string;
											break;
										case 'AddToWatchlist':
											value = filter.AddToWatchlist as boolean;
											break;
									}
									...
									if (field && value !== undefined && value !== '') {
										...
										whereParts.push(`${field}==${quotedValue}`);
									}
								}
